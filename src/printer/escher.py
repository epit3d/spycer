...
def Escher(dp: DeltaParams, points: List[Point], factorsNumber: int) -> DeltaParams:
    ...
    for i in range(K):
        ...
        dp.towerAngCorr[i] += factors[i + 3]

    return dp
    # Create a new DeltaParams object to hold the updated values
    # new_dp = DeltaParams()

    # # Update delta params with calculated factors
    # for i in range(K):
    #     new_dp.diagonals[i] = dp.diagonals[i] * factors[i]
    #     new_dp.towerPosition[i] = dp.towerPosition[i] + factors[i + 3]
